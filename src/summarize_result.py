...
def parse_argument(prog: str = __name__, description: str = 'Summarize results produced by experiments') -> dict:
	...
	parser.add_argument('--experiment', type=str, required=False, help='(Optional) Experiment name in log_dir')
	
	# Figure arguments
	parser.add_argument('--figure', action='store_true', help='Force to regenerate figures')
...
if __name__ == '__main__':
	...
	if args.experiment:
		if not path.exists(path.join(log_dir, args.experiment)): raise ValueError(f'Experiment {args.experiment} does not exist in {log_dir}')
	...
	for experiment in experiments:
		...
		log.debug(f'Print figure: {print_figure}')
		log.debug(f'Print table: {print_table}')
		...
		for metric in score.keys():
			for lambda_value in objectives:
				for g in groups:
					...
					latex_path = path.join(table_path, f'{g}_{lambda_value}.tex')
					html_path = path.join(table_path, f'{g}_{lambda_value}.html')
