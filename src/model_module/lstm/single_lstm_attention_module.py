...
class SingleLSTMAttentionModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, val=False):
        ...
        cond = (y_true > 0)
        flat_a_hat = self.flatten_attention(attention=a_hat, condition=cond, pad_mask=padding_mask, normalize='sigmoid')
        flat_a_true = self.flatten_attention(attention=a_true, condition=cond, pad_mask=padding_mask)
