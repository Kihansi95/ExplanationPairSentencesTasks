...
class SingleMlpAttention(nn.Module):
	...
	def __init__(self, d_embedding: int,
		...
		super(SingleMlpAttention, self).__init__()
		...
		n_lstm_query = kwargs.get('n_lstm_query', 1)
		self.lstm = nn.LSTM(input_size=d_in_contextualize, hidden_size=d_hidden_lstm, num_layers=n_lstm_query, batch_first=True, bidirectional=True, dropout=(n_lstm_query > 1) * dropout)
		...
		# === key block ===
		n_mlp = kwargs.get('n_mlp', 1)
		self.mlp = nn.ModuleList([
			FullyConnected(d_embedding, d_embedding, activation=activation, dropout=dropout) for _ in range(n_mlp)
		])
	...
	def forward(self, **input_):
		...
		context, attn_weights = self.attention(query=h_context, key=h_mlp, value=h_mlp, key_padding_mask=mask)
...
if __name__ == '__main__':
	...
	model = SingleMlpAttention(d_in=300, dropout=0, d_fc_lstm=-1, d_fc_attentiion=-1, d_context=-1, n_class=3, n_fc_out=0)
