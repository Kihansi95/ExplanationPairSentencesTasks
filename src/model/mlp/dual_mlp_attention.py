...
class DualMlpAttention(nn.Module):
	...
	def forward(self, premise_ids, hypothesis_ids, premise_padding=None, hypothesis_padding=None):
		...
		for s, s_bar in sides, sides[::-1]:
			...
			context[s_bar], attn_weights[s] = self.attention(query=h_context[s_bar], key=h_mlp[s], value=h_mlp[s],key_padding_mask=padding_mask[s])
...
if __name__ == '__main__':
	...
	model = DualMlpAttention(d_in=300, dropout=0, d_fc_lstm=-1, d_fc_attentiion=-1, d_context=-1, n_class=3, n_fc_out=0)
