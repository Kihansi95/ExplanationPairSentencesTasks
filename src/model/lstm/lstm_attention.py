...
class LstmAttention(Net):
	"""
		Factory class
	"""
	
	def __new__(cls, input_type: InputType, *args, **kwargs):
		if input_type == InputType.SINGLE:
			return SingleLstmAttention(*args, **kwargs)
		elif input_type == InputType.DUAL:
			return DualLstmAttention(*args, **kwargs)
		else:
...
class DualLstmAttention(Net):
	...
	def __init__(self, padding_idx: int, d_embedding: int = None, vocab_size: int = None, pretrained_embedding=None, n_class=3, **kwargs):
		"""
		...
		if pretrained_embedding is None:
			assert d_embedding is not None and vocab_size is not None, 'Provide `vocab_size` and `d_embedding` if no pretrained used'
		...
		self.concat_context = kwargs.get('concat_context',
		                                 True)  # concatenate by default, to be compatible with other script
	...
	def forward(self, premise_ids, hypothesis_ids, premise_padding=None, hypothesis_padding=None):
		...
		for s, s_bar in sides, sides[::-1]:
			...
			context[s_bar], attn_weights[s] = self.attention(query=h_last[s_bar], key=h_seq[s], value=h_seq[s],
			                                                 key_padding_mask=padding_mask[s])
...
class SingleLstmAttention(Net):
	...
	def __init__(self, padding_idx: int, d_embedding: int=None, vocab_size: int = None, pretrained_embedding=None, n_class=3,
	             **kwargs):
		"""
		Delta model has a customized attention layers
		"""
...
if __name__ == '__main__':
	...
	model = LstmAttention(d_embedding=300,
